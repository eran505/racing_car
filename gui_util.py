...
class game_gui(object):
    ...
    def draw_players(self,surface):
        for ky in self.player_dico:
            self.player_dico[ky].draw(surface,self.window_size // self.x_axis,self.window_size // self.y_axis)
...
class cube(object):
    ...
    def draw(self, surface, dis_x,dis_y):
        mergin=0
        j = self.pos[0]
        i = self.pos[1]
        ...
        pygame.draw.rect(surface, self.color, (i * dis_x + mergin , j * dis_y + mergin , dis_x -mergin , dis_y - mergin ))
